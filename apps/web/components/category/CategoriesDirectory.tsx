...
export type VolumeWindow = (typeof WINDOWS)[number]['value'];
...
export function CategoriesDirectory({
  ...
  const rows = useMemo(() => {
    ...
    return next;
  }, [live, family, sort, query, window, tick]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="nv-glass flex flex-col gap-3 rounded-[18px] p-3 lg:flex-row lg:items-center">
        <label className="flex min-w-0 flex-1 items-center gap-2 rounded-full border border-[var(--color-border-subtle)] bg-[rgba(5,9,8,0.4)] px-3">
        ...
        <select
          ...
          className="h-11 rounded-full border border-[var(--color-border-subtle)] bg-[var(--color-surface-1)] px-4 text-sm"
        >
        ...
        <button
          ...
          className="nv-icon-btn h-10 w-10"
        >
      ...
      {rows.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-[20px] border border-[var(--color-border-subtle)] bg-[color-mix(in_srgb,var(--color-surface-1)_88%,transparent)]">
          <div className="nv-category-grid hidden border-b border-[var(--color-border-subtle)] px-5 py-3 text-[10px] uppercase tracking-[0.16em] text-[var(--color-text-tertiary)] lg:grid">
            <span>#</span>
            <span>Category</span>
            <span>Type</span>
      ...
      <div className="nv-glass flex flex-wrap items-center gap-3 rounded-[16px] px-4 py-3 text-[12px] text-[var(--color-text-secondary)]">
        <span className="inline-flex items-center gap-2">
          <LiveIndicator tone={syncing ? 'amber' : 'green'} size={6} />
